...
function AadharCard() {
  ...
  useEffect(() => {
    if (selectedSubAdmin) {
      ...
      console.log("foundSubAdmin", foundSubAdmin.cscId);
      setsubadminCscId(foundSubAdmin.cscId);
    }
  }, [selectedSubAdmin]);
  // select the Subadmin asing for user function End

  console.log("selectedUserIds", selectedUserIds);
  console.log("subAdminCscId", subadminCscId);
  console.log("user", users);

  // selected the user assing for subadmin start
  useEffect(() => {
    if (selectedUserIds.length > 1) {
      const getSelectUserInfo = selectedUserIds.map((ele) => UserList(ele));
      console.log("getSelectUserInfo", getSelectUserInfo);
    }
  }, [selectedUserIds]);
                  ...
                  <td>
                    ...
                    {assignSubAdmin && (
                      <input
                        type="checkbox"
                        className="ms-3"
                        onChange={() => handleSelectUsers(index + 1)}
                        checked={selectedUserIds.includes(index + 1)}
                      />
                    )}
                  </td>
                  <td>{user?.cscId ?? ""}</td>
